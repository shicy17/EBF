...
# for nhz in [1]:
    ...
    # data = np.load('ebf1231retesteccv' + hz + str(tau) + 'taupredandtrue.npy', allow_pickle=True) 
    filename = 'ebf1231retest' + app + hz + str(tau) + 'taupredandtrue.npy'  
    ...
    for resize in [5]:
        ...
        print('***********resize************',resize)
        if True:
            for thr in [i for i in range(1,10,2)]+[i for i in range(10,160,20)]:
            # for thr in [1,2,3,4,5,6,7,8,11,13]:
                ...
                if fp>0 and (tp/fp) > 0:
                    ...
                    snr = 0
                # print('snr', snr)
                info.append([tau,thr,fpr,tpr,nerr,verr,acc,snr])
                # info[thr] = perinfo
            ...
            print('tau', tau, 'aucvalue', aucvalue)
# 把数据存储到csv中

import csv
fields = ['tau','thr', 'fpr','tpr','nerr','verr','acc','snr']
# with open(app +weights+'5hzebf0422.csv', 'w', newline='') as csvfile:
with open(app +weights+'testtau5hzinput5ebf.csv', 'w', newline='') as csvfile:
