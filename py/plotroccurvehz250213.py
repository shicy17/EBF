...
rootpath= 'D:\\code\\qmlpf-main\\qmlpf-main\\ebf\\rescsvs\\' # adjusted according to your path
...
for filename in files:
    # print(filename)
    if app in filename and weights in filename and 'filter.csv' in filename:# and application in filename:# and filtername in filename:
        
        if 'wap' in filename or 'gau' in filename:
            continue
        ...
        for idx in range(3,8,2):
            ...
            prefix = filename.split(app)[1].split('bifilter')[0]
            method = prefix.split(weights)[0] + ',' + weights.upper() + ',s='+str(idx)
...
plt.savefig(app+  'AllhzROC250213.pdf')  
